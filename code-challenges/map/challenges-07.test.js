...
const forLoopTwoToThe = (arr) => {
  // Solution code here...
  let finalArr = [];
  for (const power of arr) {
    finalArr.push(Math.pow(2,power));
  }
  return finalArr;
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let finalArr = [];
  arr.forEach(num => {finalArr.push(Math.pow(2,num));});
  return finalArr;
};
...
const evenOdd = (arr) => {
  // Solution code here...
  return arr.map(num => num % 2 === 0 ? 'even'
    : isNaN(num) ? 'N/A'
      : 'odd');
};
...
const extractStats = (arr) => {
  ...
  arr.map(stats => {
    let statsObj = {};
    statsObj['name'] = stats.stat.name;
    statsObj['total'] = stats.effort + stats.baseStat;
